...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    coffee:
      compile:
        files:
          'backbone.viewmanager.js': [ 'src/backbone.viewmanager.coffee' ]
    ...
    uglify:
      main:
        files:
          'backbone.viewmanager.min.js': [ 'backbone.viewmanager.js' ]

    karma:
      options:
        browsers: [ 'PhantomJS' ]
        frameworks: [ 'mocha', 'chai' ]
        files: [ 'backbone.viewmanager.js', 'test/**/*.coffee' ]
    ...
    replace:
      configVersion:
        src: [ 'src/backbone.viewmanager.coffee' ]
        overwrite: true
        replacements: [
          from: /Backbone\.ViewManager\.VERSION = '.*?'/i
          to: -> "Backbone.ViewManager.VERSION = '#{ packageVersion() }'"
        ]
      ...
      debugJS:
        ...
        replacements: [
          from: /^/
          to: -> "/* backbone.viewmanager.js v#{ packageVersion() }" +
            " (coffeescript output) */ \n\n"
        ]

      minJS:
        src: [ 'backbone.viewmanager.min.js' ]
        overwrite: true
        replacements: [
          from: /^/
          to: -> "/* backbone.viewmanager.min.js v#{ packageVersion() } */ \n"
        ]
    ...
    bump:
      options:
        ...
        commitFiles: [
          'package.json'
          'src/backbone.viewmanager.coffee'
          'backbone.viewmanager.js'
          'backbone.viewmanager.min.js'
        ]
        pushTo: 'origin'
